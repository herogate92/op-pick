...
export function matchesCombo(card: { searchText: string; heroes: string[]; category: string }, query: string, hero: string, category: string) {
  const normalize = (text: string) => text.toLocaleLowerCase("ko").replace(/[\s.\-:]/g, "");
  return (!hero || card.heroes.includes(hero)) && (!category || card.category === category) && query.trim().split(/\s+/).every(term => normalize(card.searchText).includes(normalize(term)));
}
