    ...
    <main className="page-shell combos-page">
      ...
      <div className="content-with-rail">
        <div className="page-content">
          <details className="combo-basis"><summary>모드별 추천 · 자료 기준 안내</summary><p>5대5·6대6을 선택해 해당 모드의 조건부 추천을 확인하세요. 6대6은 두 탱커의 역할 분담을 포함하며 고정 2/2/2 구성을 요구하지 않습니다. 공개 사례·기술 설명을 검토했으며 게임 내 직접 재현은 하지 않았습니다. 과거 사례가 현재 승률 우위를 보장하지는 않습니다.</p></details>
          <ComboExplorer heroes={heroes.map(({ key, name, role, portrait }) => ({ key, name, role, portrait }))} cards={[
            ...teamSynergies.map(synergy => ({ id: synergy.id, modes: synergy.modes, heroes: synergy.heroes, category: synergy.category, searchText: [synergy.name, synergy.type, ...synergy.abilities.map(a => a.name), ...synergy.heroes.map(key => heroSearchTerms(key, getHero(key)!.name))].join(" "), content: <SynergyCard synergy={synergy} /> })),
            ...combos.map(combo => ({ id: combo.id, modes: combo.modes, heroes: combo.heroes, category: "ultimate", searchText: [combo.name, ...combo.heroes.map(key => heroSearchTerms(key, getHero(key)!.name))].join(" "), content: <UltimateComboCard combo={combo} /> })),
          ]} />
