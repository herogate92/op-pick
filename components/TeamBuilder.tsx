...
import type { Combo, Role } from "@/lib/data";
...
export function TeamBuilder({ heroes, combos }: { heroes: BuilderHero[]; combos: Combo[] }) {
  ...
  const balanceScore = mode === "5v5"
    ? (filled ? 30 : 0)
    : (roleCounts.tank > 0 ? 9 : 0) + (roleCounts.damage > 0 ? 7 : 0) + (roleCounts.support > 0 ? 9 : 0) + (roleCounts.support >= 2 ? 6 : 0) + (roleCounts.tank <= 2 ? 4 : 0);
  const teamScore = Math.min(100, Math.round(completeness * 55 + balanceScore + Math.min(15, matchedCombos.reduce((sum, combo) => sum + combo.score * 2, 0))));
  const issues = getIssues(mode, team, roleCounts, matchedCombos.length);
  const targetSlot = team[activeSlot] === null ? activeSlot : Math.max(0, team.findIndex((key) => key === null));
  const recommendations = rankCandidates(heroes, combos, mode, team, targetSlot).slice(0, 5);
  ...
  const completeTeam = () => {
    ...
    for (let index = 0; index < draft.length; index += 1) {
      ...
      const [best] = rankCandidates(heroes, combos, mode, draft, index);
      ...
      <div className="builder-layout">
        ...
        <aside className="builder-analysis">
          <section className="analysis-score-card">
            ...
            <div><span className="section-kicker">COMPOSITION SCORE</span><h2>{filled === team.length ? "조합 평가 완료" : `${team.length - filled}자리 남음`}</h2><p>완성도, 역할 균형, 대표 궁 연계를 합산한 참고 점수입니다.</p></div>
          </section>
...
function rankCandidates(heroes: BuilderHero[], combos: Combo[], mode: Mode, team: Team, slotIndex: number) {
  ...
  return heroes.filter((hero) => !selected.includes(hero.key) && (!requiredRole || hero.role === requiredRole)).map((hero) => {
    let score = combos.filter((combo) => combo.heroes.includes(hero.key) && combo.heroes.some((key) => selected.includes(key))).reduce((sum, combo) => sum + combo.score * 12, 0);
    if (!selected.length) score += combos.filter((combo) => combo.heroes.includes(hero.key)).reduce((sum, combo) => sum + combo.score * 4, 0);
    if (mode === "6v6") {
...
function getIssues(mode: Mode, team: Team, counts: Record<Role, number>, comboCount: number) {
...
function recommendReason(hero: BuilderHero, selected: string[], combos: Combo[], counts: Record<Role, number>, mode: Mode) {
