...
export function ComboExplorer({ cards, heroes }: { cards: Card[]; heroes: PickerHero[] }) {
  ...
  return <>
    <section className="combo-filters" aria-label="조합 검색과 필터">
      ...
      <details className="combo-extra-filters"><summary>이름 검색·조합 유형{query || category ? " · 필터 적용 중" : ""}</summary>
        ...
        <div className="combo-type-buttons" aria-label="조합 유형"><button type="button" aria-pressed={!category} onClick={() => setCategory("")}>모든 유형</button>{groups.map(group => <button type="button" key={group.id} aria-pressed={category === group.id} onClick={() => setCategory(group.id)}>{group.title}</button>)}</div>
      </details>
    </section>
